...
//Set Main class in Run Configuration to assignment7.Main2
...
public class HospitalRoom {
	...
	private final Object DOCTOR_LOCK = new Object();
	private final Object PATIENT_LOCK = new Object();

	public boolean doctorEnter(Doctor d) throws InterruptedException {
		// TODO: implement your code here
		// synchronized() to ensure other threads won't use the resources.
		synchronized (DOCTOR_LOCK) {
			if (numOfDoctors != 1) {
				numOfDoctors += 1;
				System.out.println("Doctor " + d.name + " Entered, number of doctor " + numOfDoctors);
				DOCTOR_LOCK.notify();
				return true;
			} else {
				System.out.println("Doctor " + d.name + " is waiting to Enter, number of doctor " + numOfDoctors);
				DOCTOR_LOCK.wait();
				return false;
			}
		}
	...
	public boolean doctorLeave(Doctor d) throws InterruptedException {
		...
		synchronized (DOCTOR_LOCK) {
			if (numOfDoctors != 0) {
				numOfDoctors -= 1;
				System.out.println("Doctor " + d.name + " Left, number of doctor " + numOfDoctors);
				DOCTOR_LOCK.notify();
				return true;
			} else {
				DOCTOR_LOCK.wait();
	...
	public boolean patientEnter(Patient p) throws InterruptedException {
		// TODO: implement your code here
		synchronized (PATIENT_LOCK) {
			if (numOfPatients == 3) {
				System.out.println("Patient " + p.name + " is waiting to enter, number of patients " + numOfPatients);
				PATIENT_LOCK.wait();
				return false;
			} else {
				numOfPatients += 1;
				System.out.println("Patient " + p.name + " entered, number of patients " + numOfPatients);
				PATIENT_LOCK.notify();
				return true;
			}
		}
